...
def get_mock_responses_list():
    ...
    mock_responses = os.listdir(MOCK_RESPONSES_PATH)
    if args.exclude:
    ...
    if args.scan_files:
        ...
        for file_path in args.scan_files:
            with open(file_path) as f:
                scanned_files_content += f.read()
        mock_responses = [
            f for f in mock_responses if f.split(".")[0] in scanned_files_content
        ]
...
def find_coverage(properties, responses):
    ...
    for prop_name, prop_content in properties.items():
        ...
        if prop_content.get("type") == "array" and "$ref" in prop_content["items"]:
            next_responses = defaultdict(list)
            for file_name in responses:
                for part in responses[file_name]:
                    if prop_name in part:
                        # Add each element in the array as a response part
                        for elem in part[prop_name]:
                            next_responses[file_name].append(elem)
            output[prop_name].update(
                find_coverage(
